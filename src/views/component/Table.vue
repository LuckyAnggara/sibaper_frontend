  ...
  <section class="bg-white py-20 lg:py-[120px]">
    <template v-if="dataTable">
      <div class="mb-5 relative flex flex-row items-center">
        ...
        <div class="ml-5 relative sm:rounded-lg">
          <select
            v-model="limit"
            id="countries"
            class="relative mt-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          >
            <option v-for="item in limitPage" :key="item.id">{{ item }}</option>
          </select>
        </div>
      </div>
  ...
  methods: {
    getData() {
      this.$axios
        ...
        .then(res => (this.dataTable = res.data.data))
    },
    next() {
